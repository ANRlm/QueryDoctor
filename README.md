...
- AI 诊断能力（LangGraph + LLM，支持 OpenAI / DeepSeek / 通义千问）
- 慢查询自动分析与归因
- EXPLAIN 执行计划智能解析
- 索引建议生成
- SQL 优化建议输出
- SSE 实时流式响应
- WebSocket 实时通信
- RAG 知识库（基于 OpenAI Embeddings，自动积累诊断经验）
- JWT 用户认证
- 登录 / 注册 UI
- 诊断历史记录页面
- 自定义数据库连接配置（前端 UI 支持传入 `db_config`）
...
| OPENAI_API_KEY | LLM API Key（OpenAI / DeepSeek / 通义千问等） | - |
| OPENAI_BASE_URL | LLM API 地址（兼容 OpenAI 格式） | https://api.openai.com/v1 |
| OPENAI_MODEL | 使用的模型名称 | gpt-4o-mini |
| RAG_PERSIST_DIR | RAG 向量存储路径 | /data/vectorstore |
| RAG_COLLECTION | RAG 集合名称 | diagnostics |
